...
static bool decode_wpp_parallel(DecodingContext& ctx, BitstreamReader& bs,
    ...
    // so the slice spans exactly (num_entry_point_offsets + 1) rows. Rows
    // beyond endRow belong to other slices and must not be decoded here.
    int endRow = startRow + static_cast<int>(substream_byte_pos.size());
    ...
    int num_active_rows = endRow - startRow;
