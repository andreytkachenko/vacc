...
const SPS_NAL: &[u8] = &[
        0x67, 0x4d, 0x40, 0x1e, 0xec, 0xa0, 0x50, 0x17, 0xfc, 0xb8, 0x08, 0x80,
        0x00, 0x00, 0x03, 0x00, 0x80, 0x00, 0x00, 0x1e, 0x07, 0x8b, 0x16, 0xcb,
        0x00,    ];
const PPS_NAL: &[u8] = &[
        0x68, 0xeb, 0xe3, 0xcb, 0x20,];
...
// Real slice NAL units (complete coded slice header), decode order.
    ...
    const SLICE_W32_WRAP: &[u8] = &[0x41, 0x9a, 0x23, 0x49, 0xa8, 0x41, 0x6c, 0x99, 0x4c, 0x0b, 0xff];
    ...
    loop {
        match parser.parse(&pkt).expect("parse failed") {
            ...
            ParseResult::Slice { slices, .. } => {
                for s in &slices {
                    let header = s.slice_header.clone().expect("real NAL data must yield a parsed slice header");
                    match header {
...
fn test_slice_headers_real_data() {
    ...
    let headers = parse_headers(
        &mut parser,
        &[SPS_NAL, PPS_NAL, SLICE_IDR, SLICE_P1, SLICE_B2, SLICE_P3_RPLM, SLICE_P4, SLICE_B5_MMCO],
    );
    assert_eq!(headers.len(), 6);

    let (idr, p1, b2, p3, p4, b5) = (&headers[0], &headers[1], &headers[2], &headers[3], &headers[4], &headers[5]);
    ...
    assert_eq!(
        p3.ref_pic_list_modification_l0.iter().map(|e| (e.op, e.difference)).collect::<Vec<_>>(),
    ...
    assert_eq!(
        p4.ref_pic_list_modification_l0.iter().map(|e| (e.op, e.difference)).collect::<Vec<_>>(),
    ...
    let mmco: Vec<(u32, u32)> = b5.dec_ref_pic_marking.iter().map(|e| (e.memory_management_control_operation, e.value)).collect();
...
fn test_poc_wraparound() {
    ...
    let headers = parse_headers(
        ...
        &[
            SPS_NAL, PPS_NAL,
            SLICE_W29, SLICE_W30, SLICE_W31, SLICE_W32_WRAP, SLICE_W33, SLICE_W34, SLICE_W35,
        ],
    );
    assert_eq!(headers.len(), 7);
    let got = pocs(&sps, &headers);
    assert_eq!(got, [58i32, 56, 60, 70, 66, 64, 68], "wraparound POC mismatch");
}
...
fn test_full_stream_pocs() {
    ...
    let expected: [i32; 300] = [
        0, 4, 2, 6, 14, 10, 8, 12, 22, 18, 16, 20,
        30, 26, 24, 28, 38, 34, 32, 36, 46, 42, 40, 44,
        54, 50, 48, 52, 62, 58, 56, 60, 70, 66, 64, 68,
        78, 74, 72, 76, 86, 82, 80, 84, 94, 90, 88, 92,
        102, 98, 96, 100, 110, 106, 104, 108, 118, 114, 112, 116,
        126, 122, 120, 124, 134, 130, 128, 132, 142, 138, 136, 140,
        150, 146, 144, 148, 158, 154, 152, 156, 166, 162, 160, 164,
        174, 170, 168, 172, 182, 178, 176, 180, 190, 186, 184, 188,
        198, 194, 192, 196, 206, 202, 200, 204, 214, 210, 208, 212,
        222, 218, 216, 220, 230, 226, 224, 228, 238, 234, 232, 236,
        246, 242, 240, 244, 254, 250, 248, 252, 262, 258, 256, 260,
        270, 266, 264, 268, 278, 274, 272, 276, 286, 282, 280, 284,
        294, 290, 288, 292, 302, 298, 296, 300, 310, 306, 304, 308,
        318, 314, 312, 316, 326, 322, 320, 324, 334, 330, 328, 332,
        342, 338, 336, 340, 350, 346, 344, 348, 358, 354, 352, 356,
        366, 362, 360, 364, 374, 370, 368, 372, 382, 378, 376, 380,
        390, 386, 384, 388, 398, 394, 392, 396, 406, 402, 400, 404,
        414, 410, 408, 412, 422, 418, 416, 420, 430, 426, 424, 428,
        438, 434, 432, 436, 446, 442, 440, 444, 454, 450, 448, 452,
        462, 458, 456, 460, 470, 466, 464, 468, 478, 474, 472, 476,
        486, 482, 480, 484, 494, 490, 488, 492, 498, 496, 0, 8,
        4, 2, 6, 16, 12, 10, 14, 24, 20, 18, 22, 32,
        28, 26, 30, 40, 36, 34, 38, 48, 44, 42, 46, 56,
        52, 50, 54, 64, 60, 58, 62, 72, 68, 66, 70, 80,
        76, 74, 78, 88, 84, 82, 86, 96, 92, 90, 94, 98,    ];
