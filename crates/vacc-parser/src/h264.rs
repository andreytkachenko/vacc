...
pub struct H264Parser {
    sps_cache: HashMap<u32, vacc_core::picture::H264Sps>,
    pps_cache: HashMap<u32, vacc_core::picture::H264Pps>,
    active_sps: Option<vacc_core::picture::H264Sps>,
    active_pps: Option<vacc_core::picture::H264Pps>,
...
impl H264Parser {
    pub fn new() -> Self {
        Self {
            ...
            active_sps: None,
            active_pps: None,
            detected_format: DetectedVideoFormat::new(vacc_core::codec::VideoCodec::DecodeH264),
    ...
    pub fn active_sps(&self) -> Option<&vacc_core::picture::H264Sps> {
        self.active_sps.as_ref()
    }

    /// Returns a reference to the active PPS, if any.
    pub fn active_pps(&self) -> Option<&vacc_core::picture::H264Pps> {
    ...
    fn parse_vui_parameters(r: &mut BitReader) -> ParserResult<vacc_core::picture::H264SpsVui> {
        let mut vui = vacc_core::picture::H264SpsVui::default();
    ...
    fn parse_sps(&mut self, data: &[u8]) -> ParserResult<vacc_core::picture::H264Sps> {
        if data.is_empty() {
    ...
    fn parse_pps(&mut self, data: &[u8]) -> ParserResult<vacc_core::picture::H264Pps> {
        if data.is_empty() {
            return Err(ParserError::InvalidBitstream);
        }
        ...
        let mut pps = vacc_core::picture::H264Pps::new();
    ...
    fn update_format_from_sps(&mut self, sps: &vacc_core::picture::H264Sps) {
        ...
        self.detected_format.luma_bit_depth = match sps.bit_depth_luma_minus8 {
            0 => vacc_core::format::ComponentBitDepth::Bit8,
            2 => vacc_core::format::ComponentBitDepth::Bit10,
            4 => vacc_core::format::ComponentBitDepth::Bit12,
        ...
        self.detected_format.chroma_subsampling = match sps.chroma_format_idc {
            0 => vacc_core::format::ChromaSubsampling::Monochrome,
            1 => vacc_core::format::ChromaSubsampling::_420,
            2 => vacc_core::format::ChromaSubsampling::_422,
            3 => vacc_core::format::ChromaSubsampling::_444,
            _ => vacc_core::format::ChromaSubsampling::_420,
        };
    ...
    pub fn parse_slice_header(
        ...
        // num_ref_idx_active_override_flag per H.264 spec 7.4.3 (verified
        // against ffmpeg cbs_h264_syntax_template.c and x264 encoder.c).
        if is_b {
...
impl VideoParser for H264Parser {
    fn init(&mut self, format: &DetectedVideoFormat) -> ParserResult<()> {
        if format.codec != vacc_core::codec::VideoCodec::DecodeH264 {
            return Err(ParserError::InvalidBitstream);
        }
