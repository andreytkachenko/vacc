...
pub struct Av1Parser {
    ...
    active_sps: Option<vacc_core::picture::Av1Sps>,
    /// Detected format.
...
impl Av1Parser {
    pub fn new() -> Self {
        Self {
            active_sps: None,
            detected_format: DetectedVideoFormat::new(vacc_core::codec::VideoCodec::DecodeAv1),
            frame_count: 0,
    ...
    pub fn set_active_sps(&mut self, sps: vacc_core::picture::Av1Sps) {
    ...
    pub fn parse_sequence_header_obu(
        &mut self,
        obu_data: &[u8],
    ) -> ParserResult<vacc_core::picture::Av1Sps> {
        let mut sps = vacc_core::picture::Av1Sps::new();
    ...
    fn parse_timing_info(
        sps: &mut vacc_core::picture::Av1Sps,
        r: &mut BitReader,
    ) -> ParserResult<()> {
    ...
    fn update_format_from_sps(&mut self, sps: &vacc_core::picture::Av1Sps) {
        ...
        let luma_bd = if sps.twelve_bit {
            vacc_core::format::ComponentBitDepth::Bit12
        } else if sps.high_bitdepth {
            vacc_core::format::ComponentBitDepth::Bit10
        } else {
...
impl VideoParser for Av1Parser {
    fn init(&mut self, format: &DetectedVideoFormat) -> ParserResult<()> {
        if format.codec != vacc_core::codec::VideoCodec::DecodeAv1 {
            return Err(ParserError::InvalidBitstream);
        }
                            ...
                            if self.active_sps.is_none() {
                                let seq_header = self.parse_sequence_header_obu(obu_data)?;
                                 return Ok(ParseResult::ParameterSet {
                                     sps: Some(
                                         vacc_core::picture::BoxedPictureParametersSet::new(
                                             seq_header,
                                         ),
                                     ),
                                     pps: None,
