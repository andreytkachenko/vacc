...
impl<D: SurfaceMemoryDescriptor> Surface<D> {
    ...
    /// Returns a PRIME descriptor for this surface using `VA_EXPORT_SURFACE_SEPARATE_LAYERS`.
    ///
    /// Some drivers (notably the NVIDIA NVDEC VA-API driver) only implement the
    /// separate-layers export form and return `VA_STATUS_ERROR_INVALID_SURFACE`
    /// when the composed-layers flag is used. Use this variant on such drivers.
    pub fn export_prime_separate(&self) -> Result<DrmPrimeSurfaceDescriptor, VaError> {
        self.export_prime_with_flags(
            bindings::constants::VA_EXPORT_SURFACE_READ_ONLY
                | bindings::constants::VA_EXPORT_SURFACE_SEPARATE_LAYERS,
        )
    }
