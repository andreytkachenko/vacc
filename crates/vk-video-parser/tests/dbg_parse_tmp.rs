...
#[test]
fn dbg() {
    let data = std::fs::read("/home/andrey/workspace/rewrite/vk-video-worktree/nvdev/assets/bframe_test.h264").unwrap();
            ...
            Ok(ParseResult::Slice { slices, .. }) => {
                ...
                if let Some(vk_video_parser::SliceHeader::H264(h)) = &slices[0].slice_header {
                    if pic < 12 {
                        ...
                        println!("  rplm_l0={:?} rplm_l1={:?}", h.ref_pic_list_modification_l0, h.ref_pic_list_modification_l1);
                        println!("  mmco={:?}", h.dec_ref_pic_marking);
                        println!("  cabac={} qpd={} header_bits={}", h.cabac_init_idc, h.slice_qp_delta, h.header_bit_size);
                    }
                }
                pic += 1;
                if pic > 12 { break; }
