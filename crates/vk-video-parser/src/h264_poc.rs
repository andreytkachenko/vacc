//! H.264 Picture Order Count calculator (common decode-state foundation).
//!
...
pub struct PocCalculator {
    ...
    has_prev_pic: bool,
    // POC Type 2 state: track FrameNum wrap cycles to produce monotonic POCs.
    // When FrameNum wraps (e.g., 15→0), the raw POC (FrameNum*2) wraps too.
    // We track the cycle count so that POCs remain monotonic across wraps.
    frame_num_cycle: i32,
}
...
impl PocCalculator {
    ...
    pub fn calculate(&mut self, sps: &H264Sps, slh: &SliceHeader, is_reference: bool) -> i32 {
        ...
        let poc = match sps.pic_order_cnt_type {
            ...
            2 => self.calc_type2(sps, slh, is_reference),
            _ => 0,
        };
    ...
    ///
    /// When FrameNum wraps (e.g., 15→0), the raw POC wraps too. We track the
    /// wrap cycle so that POCs remain monotonic across wraps, which is required
    /// for correct presentation-order sorting and B-slice reference lists.
    fn calc_type2(&mut self, sps: &H264Sps, slh: &SliceHeader, is_reference: bool) -> i32 {
        let frame_num = slh.frame_num as i32;
        let max_frame_num = 1i32 << (sps.log2_max_frame_num_minus4 as i32 + 4);

        // Detect FrameNum wrap: if frame_num decreased significantly, it wrapped.
        if self.has_prev_pic && frame_num < self.prev_frame_num {
            self.frame_num_cycle += 1;
        }
