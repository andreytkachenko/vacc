//! NVDEC VP9 decode example with pixel output and params dump.
//!
//! Demonstrates hardware-accelerated VP9 decoding using NVIDIA NVDEC, driven
//! by the Rust `vk-video-parser` (Vp9Parser) via [`NvdecVp9Decoder`]:
//!
//! 1. Load a VP9 IVF file (or raw single frame)
//! 2. Create the NVDEC decoder (IVF + superframe expansion, DPB management)
...
//!
//! Usage:
//!   cargo run --example decode_nvdec_vp9 -- \
//!     [ivf_file] [max_frames] [dump.txt] [out_prefix]
//!   defaults: assets/big_buck_bunney_vp9.ivf 300 None nvdec_vp9

use nvdec_decode::NvdecVp9Decoder;
...
fn main() {
    ...
    let out_prefix: String = args
        .get(4)
        .cloned()
        .unwrap_or_else(|| "nvdec_vp9".to_string());
    ...
    println!("=== NVDEC VP9 Decode Example ===");
    ...
    println!("\n--- Creating NVDEC VP9 decoder ---");
    let mut decoder = match NvdecVp9Decoder::new(data) {
        Ok(d) => d,
        Err(e) => {
    ...
    if frames_decoded > 0 {
        println!("Success: NVDEC VP9 decoding with pixel output is working!");
    } else {
