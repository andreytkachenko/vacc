...
fn main() {
    ...
    let want_pic: usize = std::env::args().nth(2).map(|s| s.parse().unwrap()).unwrap_or(2);
        ...
        match parser.parse(&packet) {
            Ok(ParseResult::Slice { slices, .. }) => {
                ...
                if pic == want_pic {
                    for sl in &slices {
                        let nal = &sl.nal_data;
                        println!("pic{} nal ({} bytes): {:02x?}", pic, nal.len(), &nal[..nal.len().min(48)]);
                        // EPB removal
                        ...
                        if let Some(SliceHeader::H264(slh)) = &sl.slice_header {
                            println!("parsed: fn={} st={} mmco=[{}]", slh.frame_num, slh.slice_type,
                                slh.dec_ref_pic_marking.iter().map(|e| format!("op{}={}", e.memory_management_control_operation, e.value)).collect::<Vec<_>>().join(","));
                        }
                    }
                    break;
                }
